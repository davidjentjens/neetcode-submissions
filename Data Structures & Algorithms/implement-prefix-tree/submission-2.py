...
class PrefixTree:
    ...
    def insert(self, word: str) -> None:
        def dfs(word_part, node, prev_word_found):
            ...
            if not prev_word_found:
                new_node = TreeNode(word_part[0], [], False)
                node.children.append(new_node)
                dfs(word_part[1:], new_node, False)
                return
        ...
        dfs(word, self.root, True)

    def search(self, word: str) -> bool:
        def dfs(word_part, node):
            if len(word_part) == 0:
                return node.end_of_word
            
            found_node = None
            for node in node.children:
                if word_part[0] == node.val:
                    found_node = node

            if found_node:
                return dfs(word_part[1:], found_node)
            return False
